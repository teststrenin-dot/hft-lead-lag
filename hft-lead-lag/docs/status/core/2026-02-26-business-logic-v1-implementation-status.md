...
| `HFT-CP3` Event-Driven Updated-Only Processing | `In Progress` | Updated-symbol batch de-duplicates without sort; pending-signal queue now uses `SymbolId` bitset (`PendingSymbolSet`) | Still needs clone elimination in strategy feed path and full rate-proportional processing proof |
