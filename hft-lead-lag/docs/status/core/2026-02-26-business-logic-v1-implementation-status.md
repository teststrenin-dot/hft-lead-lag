...
| `HFT-CP4` Minimal-Copy WS Parse Path | `In Progress` | ticks and fast parse are in place; symbol cache now interns raw bytes directly (no UTF-8 fallback conversion path) | Remaining parse/copy hot spots need profiling-backed cleanup |
