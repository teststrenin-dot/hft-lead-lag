...
| `HFT-CP4` Minimal-Copy WS Parse Path | `In Progress` | Fast parsing exists; symbol cache interns raw bytes directly; dynamic field-name extractors are test-only in runtime build; remaining parse/copy hot spots still need cleanup. |
