...
| `HFT-CP3` Event-Driven Updated-Only Processing | `In Progress` | Process only updated symbols via bitset/updated-id set | Pending-symbol bitset path is live; strategy-update queue now carries tickers directly (no runtime cache lookup clone); finalize rate-proportional proof |
