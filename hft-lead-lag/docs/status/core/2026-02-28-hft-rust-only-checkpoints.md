...
|---|---|---|---|
| `HFT-CP0` Latency and Allocation Observatory | `In Progress` | Stage timestamps, internal latency histograms, drop counters, backlog depth | One endpoint shows p50/p95/p99/max and drop/backlog metrics; baseline captured |
| `HFT-CP1` SymbolId and Allocation Removal | `Planned` | Replace `String`/`HashMap<String,...>` in hot path with `SymbolId` and array-style state | Hot-path profile no longer dominated by `String` allocation/sort/dedup |
