...
| `HFT-CP4` Minimal-Copy WS Parse Path | `In Progress` | Parse only required fields, map symbol to id early, avoid symbol copying in hot path | Symbol cache now interns raw bytes (no UTF-8 fallback); Binance/Gate parse paths now assign `strategy_symbol_id` during parse; drain dedupe is `SymbolId`-first; proof tracked in `2026-02-28-cp4-parse-path-evidence.md`; finish remaining profile-backed cleanup |
