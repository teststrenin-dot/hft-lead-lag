...
impl SymbolState {
        ...
        match exchange {
            "binance" => {
                ...
                if let Some(v) = ingress_ws_drift {
                    self.drifts.binance_ingress = Some(v);
                }
            }
            "gate" => {
                ...
                if let Some(v) = ingress_ws_drift {
                    self.drifts.gate_ingress = Some(v);
                }
            }
            _ => return false,
        }
    ...
    pub(crate) fn update_cycles(&mut self, exchange_ts_ms: i64, window_ms: i64) {
        let (Some(binance), Some(gate)) = (self.binance.as_ref(), self.gate.as_ref()) else {
            return;
        };
        ...
        self.binance_leads
            .update(exchange_ts_ms, binance_div_bps, binance_conv_bps, window_ms);
        self.gate_leads
            .update(exchange_ts_ms, gate_div_bps, gate_conv_bps, window_ms);

        let mut means = Vec::with_capacity(2);
        if let Some(v) = self.binance_leads.average_half_life_ms() {
            means.push(v);
        }
        if let Some(v) = self.gate_leads.average_half_life_ms() {
            means.push(v);
        }
        self.entry_half_life_ms = if means.is_empty() {
            0.0
        } else {
        ...
        if let Some(v) = self.binance_leads.average_above_p90_ms() {
            gt_p90_means.push(v);
        }
        if let Some(v) = self.gate_leads.average_above_p90_ms() {
            gt_p90_means.push(v);
        }
        self.avg_gt_p90_ms = if gt_p90_means.is_empty() {
            0.0
        } else {
